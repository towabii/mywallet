...
import { state, recalcBalances, triggerRender, parseDateSafe } from './utils.js';
...
export const renderHistory = () => {
    ...
    // 修正：parseDateSafe を使って安全にソート
    txs.sort((a, b) => parseDateSafe(b.date) - parseDateSafe(a.date)).forEach(tx => {
...
export const renderCharts = () => {
    // 修正：Chart.jsが読み込まれていない場合はスキップ（クラッシュ防止）
    if (typeof Chart === 'undefined') return;

    const { transactions } = state.currentUser.data;
    const validTx = transactions.filter(t => !t.isScheduled);
    const sTxs = [...validTx].sort((a, b) => parseDateSafe(a.date) - parseDateSafe(b.date));
    ...
    if (sTxs.length > 0) {
        ...
        // 修正：文字列からの Date 生成をやめ、parseDateSafeを使用
        let d = parseDateSafe(startDStr);
        const endD = parseDateSafe(endDStr);
        
        // 無限ループ防止用の安全措置
        let loopCount = 0;

        while (d <= endD && loopCount < 3650) { // 最大10年分
            loopCount++;
    ...
    Object.keys(pData).forEach(cId => {
        if (cId === 'unknown') {
            ...
            if (c) { 
                pLbls.push(c.name); pVals.push(pData[cId]); pCols.push(c.color); 
            } else {
                // 修正：カテゴリ削除済みの場合のフォールバック
                pLbls.push('削除済カテゴリ'); pVals.push(pData[cId]); pCols.push('#95a5a6'); 
            }
        }
    });
